...
void Sound_Init(void){ 
	...
	GPIO_PORTA_AMSEL_R &= ~0x0C;      // disable analog functionality on PA3 and PA2
...
int main(void){// activate grader and set system clock to 80 MHz
  ...
  while(1){
    // main program is free to perform other tasks
		...
		in = GPIO_PORTA_DATA_R&0x08; // read PA3
		if (in == 0x08) {            // PA3 pressed
			if (prev != in) {				
				out ^= 1;                // update out on/off
				prev = in;             
			}
		} else {  									 // PA3 released
			if (prev != in) {					
